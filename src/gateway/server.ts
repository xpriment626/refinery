...
import { getMemoryGraphStatus, inspectMemoryGraphNode, planMemoryGraph } from "../core/graph/service.ts";
import { LibsqlGraphStore } from "../core/graph/libsql-store.ts";
import type { ResponsibilityPlan } from "../core/graph/plan.ts";
...
import { createGatewayEventBus, type GatewayEventBus } from "./conductor-seam.ts";

const gatewayApiVersion = "refinery.gateway.v1" as const;
...
function secureHeaders(api: boolean): Record<string, string> {
  return {
...
function writeError(response: http.ServerResponse, status: number, code: string, message: string, details?: Record<string, unknown>): void {
  writeJson(response, status, { ok: false, error: { code, message, ...(details ? { details } : {}) } });
}

function publicEdge(edge: { id: string; sourceNodeId: string; targetNodeId: string; kind: string; confidence: number }): Record<string, unknown> {
  return { id: edge.id, source: edge.sourceNodeId, target: edge.targetNodeId, kind: edge.kind, confidence: edge.confidence };
}

function publicPlan(plan: ResponsibilityPlan): Record<string, unknown> {
  return {
...
function writePublicGatewayFailure(response: http.ServerResponse, error: unknown, statusHint?: number): void {
...
async function readJsonBody(request: http.IncomingMessage): Promise<Record<string, unknown>> {
  ...
  if (declaredLength > MAX_BODY_BYTES) throw Object.assign(new Error("request body exceeds 64KB"), { status: 413, code: "REQUEST_TOO_LARGE" });
  const contentType = String(request.headers["content-type"] ?? "").split(";", 1)[0]?.trim();
  if (contentType !== "application/json") throw Object.assign(new Error("application/json is required"), { status: 415, code: "CONTENT_TYPE_REQUIRED" });
  const chunks: Buffer[] = [];
  let size = 0;
  for await (const chunk of request) {
    const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
    size += buffer.length;
    if (size > MAX_BODY_BYTES) throw Object.assign(new Error("request body exceeds 64KB"), { status: 413, code: "REQUEST_TOO_LARGE" });
    chunks.push(buffer);
  }
  let parsed: unknown;
  try {
...
export function createGatewayServer(options: CreateGatewayServerOptions): GatewayServer {
  ...
  const eventClients = new Set<{ response: http.ServerResponse; unsubscribe: () => void }>();
  let listenAddress: GatewayListenAddress | null = null;
  let closing = false;
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      if (url.pathname === "/api/v1/events") {
        if (request.method !== "GET") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Use GET for gateway events.");
        ...
        let client: { response: http.ServerResponse; unsubscribe: () => void };
      ...
      if (url.pathname === "/api/v1/health") {
        if (request.method !== "GET") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Use GET for gateway health.");
      ...
      if (url.pathname === "/api/v1/graph/status") {
        if (request.method !== "GET") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Use GET for graph status.");
      ...
      if (url.pathname === "/api/v1/graph/snapshot") {
        if (request.method !== "GET") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Use GET for graph snapshots.");
        ...
        if (!Number.isFinite(maxNodes) || !Number.isFinite(maxEdges)) return writeError(response, 400, "INVALID_LIMIT", "Snapshot limits must be finite numbers.");
        ...
        try {
          ...
          if (!snapshot) return writeError(response, 404, "GRAPH_INDEX_NOT_FOUND", "Run refinery graph sync before opening the graph UI.", { next: "refinery graph sync --json" });
      ...
      if (url.pathname === "/api/v1/graph/delta") {
        if (request.method !== "GET") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Use GET for graph deltas.");
        const after = Number(url.searchParams.get("after") ?? 0);
        if (!Number.isSafeInteger(after) || after < 0) return writeError(response, 400, "INVALID_SEQUENCE", "Delta sequence must be a non-negative safe integer.");
        const paths = resolveRefineryPaths({ home, cwd: project });
        const store = new LibsqlGraphStore(paths.graphIndexPath, { legacyJsonPath: paths.legacyGraphIndexPath });
        try {
      ...
      if (url.pathname.startsWith("/api/v1/graph/node/")) {
        if (request.method !== "GET") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Use GET for node inspection.");
        const nodeId = decodeURIComponent(url.pathname.slice("/api/v1/graph/node/".length));
        if (!nodeId || nodeId.length > 512) return writeError(response, 400, "INVALID_NODE_ID", "Node id is missing or too long.");
      ...
      if (url.pathname === "/api/v1/graph/plan") {
        if (request.method !== "POST") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Use POST for graph planning.");
        const body = await readJsonBody(request);
        if (typeof body.request !== "string" || body.request.length > 20_000) return writeError(response, 400, "INVALID_REQUEST", "Plan request must be a string no longer than 20,000 characters.");
        const seeds = Array.isArray(body.seeds) ? body.seeds.filter((seed): seed is string => typeof seed === "string").slice(0, 20) : [];
      ...
      if (url.pathname === "/api/v1/graph/changes") {
        if (request.method !== "GET") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Use GET for graph changes.");
        const after = Number(url.searchParams.get("after") ?? 0);
        if (!Number.isSafeInteger(after) || after < 0) return writeError(response, 400, "INVALID_SEQUENCE", "Change sequence must be a non-negative safe integer.");
        const paths = resolveRefineryPaths({ home, cwd: project });
        const store = new LibsqlGraphStore(paths.graphIndexPath, { legacyJsonPath: paths.legacyGraphIndexPath });
        try {
      ...
      if (url.pathname === "/api/v1/events/publish") {
        if (request.method !== "POST") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Use POST to publish an event.");
        const body = await readJsonBody(request);
        if (body.type !== "graph-synced") return writeError(response, 400, "INVALID_EVENT", "Only graph-synced events are accepted.");
        const incomingPayload = typeof body.payload === "object" && body.payload ? body.payload as Record<string, unknown> : {};
        const changed = typeof incomingPayload.changed === "object" && incomingPayload.changed ? incomingPayload.changed as Record<string, unknown> : {};
      ...
      if (url.pathname === "/api/v1/shutdown") {
        if (request.method !== "POST") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Use POST to stop the gateway.");
        if (closing) return writeJson(response, 202, { ok: true, stopping: true });
        closing = true;
      ...
      if (request.method !== "GET" && request.method !== "HEAD") return writeError(response, 405, "METHOD_NOT_ALLOWED", "Only GET and HEAD are supported for UI assets.");
    ...
    listen(port = 0) {
      return new Promise((resolve, reject) => {
        ...
        server.listen(port, "127.0.0.1", () => {
          ...
          if (!address || typeof address === "string") return reject(new Error("gateway did not receive a TCP address"));
