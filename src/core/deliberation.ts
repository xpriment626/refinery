...
export function buildDeliberationArtifacts(args: {
  ...
  const claimScoutMessages = args.messages.filter((message) => (
    message.status === "succeeded" && message.output && message.step === "claim-scout"
  ));
  ...
  const claimRows = claimScoutMessages.flatMap((message) => (
    records(message.output?.candidates).map((candidate) => ({ candidate, message }))
  ));
  const claims = claimRows.map(({ candidate, message }, index): ClaimCard => ({
