...
import { startSetupHttpServer, setupCapabilityHash, setupProtocolVersion } from "./server.ts";
...
interface SetupDaemonState {
  ...
  protocol: typeof setupProtocolVersion;
  instanceId: string;
...
function privateStateWrite(file: string, state: SetupDaemonState): void {
...
function validState(value: unknown): value is SetupDaemonState {
  if (!value || typeof value !== "object") return false;
  const state = value as Partial<SetupDaemonState>;
...
function quarantineState(file: string): void {
...
function readState(file: string): SetupDaemonState | null {
  try {
...
function publicState(state: SetupDaemonState, running: boolean): Record<string, unknown> {
  return {
    running,
...
async function health(state: SetupDaemonState): Promise<boolean> {
  if (state.port <= 0 || Date.parse(state.expiresAt) <= Date.now()) return false;
...
function setupDaemonEnvironment(env: NodeJS.ProcessEnv | Record<string, string | undefined>): NodeJS.ProcessEnv {
  const allowed = [
...
export async function setupLifecycleStatus(options: SetupLifecycleOptions): Promise<Record<string, unknown>> {
  ...
  if (!state) return { running: false, protocol: setupProtocolVersion, project, expiresAt: null };
  const running = await health(state);
  if (!running) quarantineState(paths.setupStatePath);
  return publicState(state, running);
}

export async function stopSetupLifecycle(options: SetupLifecycleOptions): Promise<Record<string, unknown>> {
  const project = path.resolve(options.project);
  const paths = resolveRefineryPaths({ home: options.home, cwd: project, env: options.env });
  const state = readState(paths.setupStatePath);
  if (!state) return { running: false, protocol: setupProtocolVersion, project, stopped: false };
  ...
  if (fs.existsSync(paths.setupStatePath)) quarantineState(paths.setupStatePath);
  return { ...publicState(state, false), stopped: true };
}

export async function startSetupLifecycle(options: SetupLifecycleOptions): Promise<Record<string, unknown>> {
  ...
  if (state.codexHome) args.push("--codex-home", state.codexHome);
...
export async function serveSetupLifecycle(options: SetupLifecycleOptions & { instanceId: string }): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    startSetupHttpServer({
      ...
      onClosed: () => {
        const current = readState(paths.setupStatePath);
        if (current?.instanceId === state.instanceId) fs.rmSync(paths.setupStatePath, { force: true });
        resolve();
      },
    }).catch(reject);
  });
