...
import { asRefineryError, RefineryError } from "../core/errors.ts";
...
export const setupProtocolVersion = "refinery.setup-gateway.v1" as const;
...
export interface SetupServerOptions {
  ...
  onListening?: (address: { host: "127.0.0.1"; port: number; pid: number }) => void;
  onClosed?: () => void;
...
function secureHashMatch(token: string, expectedHash: string): boolean {
...
export async function startSetupHttpServer(options: SetupServerOptions): Promise<RunningSetupServer> {
  ...
  const server = http.createServer(async (request, response) => {
    try {
      ...
      if (url.pathname === "/api/v1/complete") {
        ...
        if (
          typeof body.coralApiKey !== "string"
        ...
        if (options.shutdownAfterComplete !== false) setTimeout(() => void close(), 250);
        return;
      }
      sendJson(response, 404, { ok: false, error: { code: "NOT_FOUND", message: "Not found." } });
    } catch (error) {
...
export function setupCapabilityHash(capability: string): string {
