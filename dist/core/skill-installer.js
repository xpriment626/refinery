...
const managedManifestSchemaVersion = "refinery.managed-skill.v1";
...
function walkTree(root, relative = []) {
    ...
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
        ...
        if (segments.length === 1 && entry.name === managedManifestName)
            continue;
        if (entry.isDirectory())
            files.push(...walkTree(root, segments));
        else if (entry.isFile())
            files.push({ segments, kind: "file" });
        else
            files.push({ segments, kind: entry.isSymbolicLink() ? "symlink" : "special" });
    }
    return files;
}
function hashSkillTreeWithSeparator(root, separator) {
    ...
    for (const { entry, logicalPath } of entries) {
        ...
        if (entry.kind === "file")
            hash.update(fs.readFileSync(path.join(root, ...entry.segments)));
        hash.update("\0");
    }
...
function manifestMatchesTree(root, manifest, canonicalTreeHash) {
    if (!manifest)
        return false;
    if (manifest.installedTreeHash === canonicalTreeHash)
        return true;
...
function copyDirectory(source, destination) {
    ...
    for (const entry of fs.readdirSync(source, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            ...
            throw new RefineryError("SKILL_BUNDLE_UNSAFE", `Bundled skill contains a non-file entry: ${sourcePath}`, { phase: "skill-install" });
    }
}
function readManagedManifest(destination) {
    const manifestPath = path.join(destination, managedManifestName);
    try {
        const parsed = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
        if (parsed.schemaVersion !== managedManifestSchemaVersion
...
function writeManagedManifest(destination, packageVersion, installedTreeHash) {
    const manifest = {
...
function replaceManagedDirectory(source, destination, packageVersion, bundledTreeHash) {
