...
function compactResponsibilityPlan(plan) {
    const exclusionCounts = plan.exclusions.reduce((counts, exclusion) => {
...
function graphContext(index, plan) {
    ...
    return plan.selectedNodes.map((selected) => {
        ...
        if (!node || !revision || node.currentRevisionId !== revision.id) {
            throw new RefineryError("GRAPH_CONTEXT_INVALID", `Responsibility plan references unavailable node or revision: ${selected.nodeId} / ${selected.revisionId}`, { phase: "graph-context", details: { nodeId: selected.nodeId, revisionId: selected.revisionId } });
        }
        return {
            nodeId: node.id,
...
function sourceChunks(context, charLimit) {
    let remaining = charLimit;
    const chunks = [];
    for (const node of context) {
        if (node.kind === "project" || remaining <= 0)
            continue;
...
function activeMemoryHints(context, limit) {
    return context
