...
import { JsonGraphStore, memoryGraphIndexerVersion, memoryGraphSchemaVersion, } from "./sync.js";
export const graphDatabaseSchemaVersion = 3;
const SCHEMA_SQL = `
...
function parseJson(value, label) {
    if (typeof value !== "string")
        throw new Error(`${label} is not stored as text`);
    try {
...
function deriveDelta(previous, current) {
...
function visualEdge(row) {
    return {
        ...
        kind: asString(row.kind, "visual edge kind"),
...
export class LibsqlGraphStore {
    ...
    `);
        const currentRow = database.prepare("SELECT COALESCE(MAX(version), 0) AS version FROM graph_migrations").get();
        const currentVersion = asNumber(currentRow.version, "migration version");
        if (currentVersion > graphDatabaseSchemaVersion) {
            database.close();
            throw new RefineryError("GRAPH_SCHEMA_UNSUPPORTED", `Graph database schema ${currentVersion} is newer than supported schema ${graphDatabaseSchemaVersion}. Upgrade Refinery before opening it.`, { phase: "graph-migration", details: { graphPath: this.location, currentVersion, supportedVersion: graphDatabaseSchemaVersion } });
        }
        const migrations = [
    ...
    #importLegacyIfNeeded() {
        ...
        if (this.#hasGraph(database) || !this.legacyJsonPath || !fs.existsSync(this.legacyJsonPath))
            return;
        const legacy = new JsonGraphStore(this.legacyJsonPath).read();
        if (!legacy)
            return;
        this.write(legacy, null);
        database.prepare("INSERT OR REPLACE INTO graph_meta(key, value) VALUES ('legacy_imported', 'true')").run();
    ...
    read() {
        try {
            ...
            const database = this.#open();
            const metaRows = database.prepare("SELECT key, value FROM graph_meta").all();
            ...
            if (!project)
                return null;
            const nodes = database.prepare("SELECT * FROM graph_nodes ORDER BY id").all().map((row) => ({
                ...
                kind: asString(row.kind, "node kind"),
                ...
                metadata: parseJson(row.metadata_json, "node metadata"),
            }));
            const revisions = database.prepare("SELECT * FROM graph_revisions ORDER BY id").all().map((row) => ({
                ...
                indexerVersion: asString(row.indexer_version, "revision indexer version"),
            ...
            const edges = database.prepare("SELECT * FROM graph_edges ORDER BY id").all().map((row) => ({
                ...
                kind: asString(row.kind, "edge kind"),
                ...
                provenance: parseJson(row.provenance_json, "edge provenance"),
            }));
            const sourceSpecs = database.prepare("SELECT spec FROM graph_source_specs ORDER BY spec").all()
                .map((row) => asString(row.spec, "source spec"));
            return {
                schemaVersion: (meta.get("schema_version") ?? memoryGraphSchemaVersion),
                indexerVersion: (meta.get("indexer_version") ?? memoryGraphIndexerVersion),
                project,
                sourceSpecs,
            ...
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_INDEX_INVALID", `Could not read Refinery graph database at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-store", details: { graphPath: this.location } });
        }
    }
    write(index, previous = null, delta) {
        try {
            const database = this.#open();
        ...
        `);
                for (const node of index.nodes) {
                    if (!changed(previousNodes.get(node.id), node))
                        continue;
                    upsertNode.run(node.id, node.sourceAdapter, node.sourceKey, node.kind, node.scope, node.project, node.label, node.uri, node.currentRevisionId, json(node.metadata));
                }
        ...
        `);
                for (const revision of index.revisions) {
                    if (!changed(previousRevisions.get(revision.id), revision))
                        continue;
                    insertRevision.run(revision.id, revision.nodeId, revision.contentHash, revision.indexerVersion, revision.content, revision.charCount, revision.indexedAt, revision.sourceModifiedAt);
                }
                ...
                for (const nodeId of [...recordedDelta.removedNodeIds, ...changedSearchNodeIds])
                    deleteSearch.run(nodeId);
                const insertSearch = database.prepare("INSERT INTO graph_search(node_id, label, content, metadata) VALUES (?, ?, ?, ?)");
                ...
                for (const id of previousRevisions.keys())
                    if (!currentRevisionIds.has(id))
                        deleteRevision.run(id);
        ...
        `);
                for (const edge of index.edges) {
                    if (!changed(previousEdges.get(edge.id), edge))
                        continue;
                    upsertEdge.run(edge.id, edge.sourceNodeId, edge.targetNodeId, edge.kind, edge.sourceRevisionId, edge.confidence, json(edge.provenance));
                }
                ...
                for (const spec of index.sourceSpecs)
                    insertSpec.run(spec);
        ...
        `).run();
                ...
                if (foreignKeyFailures.length > 0)
                    throw new Error("graph database foreign-key integrity check failed");
            });
            ...
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_STORE_WRITE_FAILED", `Could not write Refinery graph database at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-store", details: { graphPath: this.location } });
        }
    }
    diagnostics() {
        try {
            this.#importLegacyIfNeeded();
            const database = this.#open();
            const row = database.prepare("SELECT MAX(version) AS version FROM graph_migrations").get();
            const imported = database.prepare("SELECT value FROM graph_meta WHERE key = 'legacy_imported'").get();
            const change = database.prepare("SELECT COALESCE(MAX(sequence), 0) AS sequence FROM graph_changes").get();
            return {
            ...
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_INDEX_INVALID", `Could not inspect Refinery graph database at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-store", details: { graphPath: this.location } });
        }
    }
    readChanges(options = {}) {
        try {
      ...
      `).all(afterSequence, limit);
            return rows.map((row) => ({
                ...
                delta: normalizedDelta(parseJson(row.delta_json, "change delta") ?? EMPTY_DELTA),
            }));
        }
        catch (error) {
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_INDEX_INVALID", `Could not read Refinery graph changes at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-store", details: { graphPath: this.location } });
        }
    }
    findCurrentNode(identifier) {
        try {
            const database = this.#open();
      ...
      `).get(identifier, identifier, identifier);
            ...
            const revisionRow = database.prepare("SELECT * FROM graph_revisions WHERE id = ?").get(node.currentRevisionId);
            if (!revisionRow) {
            ...
            const revision = {
                ...
                indexerVersion: asString(revisionRow.indexer_version, "revision indexer version"),
            ...
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_INDEX_INVALID", `Could not query Refinery graph node at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-store", details: { graphPath: this.location, identifier } });
        }
    }
    findFirstEligibleNode(projectInput, scope) {
        try {
            const database = this.#open();
      ...
      `).get(...parameters);
            ...
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_INDEX_INVALID", `Could not query a fallback Refinery graph node at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-search", details: { graphPath: this.location } });
        }
    }
    readAdjacentEdges(options) {
        try {
            if (options.edgeKinds.length === 0 || options.limit <= 0)
        ...
        `;
            ...
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_INDEX_INVALID", `Could not query Refinery graph adjacency at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-store", details: { graphPath: this.location, nodeId: options.nodeId } });
        }
    }
    searchNodeIds(options) {
        try {
            const tokens = [...new Set(options.request.toLowerCase().match(/[\p{L}\p{N}_-]+/gu)
      ...
      `).all(...parameters);
            ...
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_INDEX_INVALID", `Could not search Refinery graph at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-search", details: { graphPath: this.location } });
        }
    }
    readMetadata() {
        try {
            ...
            return {
                ...
                indexerVersion: (meta.get("indexer_version") ?? memoryGraphIndexerVersion),
                syncedAt: meta.get("synced_at") ?? "",
                sourceSpecs: database.prepare("SELECT spec FROM graph_source_specs ORDER BY spec").all()
                    .map((row) => asString(row.spec, "source spec")),
                counts: {
            ...
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_INDEX_INVALID", `Could not query Refinery graph metadata at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-store", details: { graphPath: this.location } });
        }
    }
    readVisualizationSnapshot(options) {
      ...
      `).all(maxNodes, maxEdges);
            ...
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_INDEX_INVALID", `Could not build Refinery graph visualization at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-visualization", details: { graphPath: this.location } });
        }
    }
    readVisualizationDelta(options) {
      ...
      `).get();
        ...
        `).all(...ids);
            ...
            if (error instanceof RefineryError)
                throw error;
            throw new RefineryError("GRAPH_INDEX_INVALID", `Could not build Refinery graph visualization delta at ${this.location}: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-visualization", details: { graphPath: this.location } });
        }
    }
}
