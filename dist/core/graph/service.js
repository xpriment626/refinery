...
import { buildCodexGraphSnapshot } from "../../sources/codex-graph.js";
import { parseSourceSpecs } from "../packets.js";
...
import { attachResponsibilityContext } from "./context.js";
import { LibsqlGraphStore } from "./libsql-store.js";
import { createResponsibilityPlan, } from "./plan.js";
import { memoryGraphEdgeKinds, memoryGraphIndexerVersion, memoryGraphSchemaVersion, syncMemoryGraph, } from "./sync.js";
...
function resolvedGraphPath(args) {
    if (args.graphPath)
        return path.resolve(args.graphPath);
    return resolveRefineryPaths({ cwd: path.resolve(args.project), home: args.home }).graphIndexPath;
}
function graphStore(args) {
...
function readRequiredGraph(args) {
    const store = args.store;
    const index = store.read();
    if (!index) {
        throw new RefineryError("GRAPH_INDEX_NOT_FOUND", `Refinery graph index not found at ${store.location}. Run refinery graph sync first.`, { phase: "graph-read", details: { graphPath: store.location, next: "refinery graph sync --json" } });
    }
    const project = path.resolve(args.project);
    if (index.project !== project) {
        throw new RefineryError("GRAPH_PROJECT_MISMATCH", `Graph index belongs to ${index.project}, not ${project}.`, { phase: "graph-read", details: { graphPath: store.location, indexProject: index.project, requestedProject: project } });
    }
    return index;
}
function assertGraphAvailable(store, projectInput, phase) {
    const metadata = store.readMetadata();
    const project = path.resolve(projectInput);
    if (!metadata) {
        throw new RefineryError("GRAPH_INDEX_NOT_FOUND", `Refinery graph index not found at ${store.location}. Run refinery graph sync first.`, { phase, details: { graphPath: store.location, next: "refinery graph sync --json" } });
    }
    if (metadata.project !== project) {
        throw new RefineryError("GRAPH_PROJECT_MISMATCH", `Graph index belongs to ${metadata.project}, not ${project}.`, { phase, details: { graphPath: store.location, indexProject: metadata.project, requestedProject: project } });
    }
}
function assertNodeInProjectScope(node, projectInput, phase) {
    const project = path.resolve(projectInput);
    if (node.scope === "global" || node.project === project)
        return;
    throw new RefineryError("GRAPH_NODE_OUT_OF_SCOPE", `Graph node ${node.id} is outside the requested project scope.`, {
        phase,
        details: {
            nodeId: node.id,
...
export async function syncCodexMemoryGraph(args) {
    ...
    try {
        ...
        if (error instanceof RefineryError)
            throw error;
        throw new RefineryError("GRAPH_SOURCE_LOAD_FAILED", `Could not load graph sources: ${error instanceof Error ? error.message : String(error)}`, { phase: "graph-source" });
    }
...
export function getMemoryGraphStatus(args) {
    ...
    store.close();
    if (metadata.project !== project) {
        throw new RefineryError("GRAPH_PROJECT_MISMATCH", `Graph index belongs to ${metadata.project}, not ${project}.`, { phase: "graph-status", details: { graphPath, indexProject: metadata.project, requestedProject: project } });
    }
    return {
        ok: true,
        command: "graph status",
...
export function inspectMemoryGraphNode(args) {
    ...
    try {
        ...
        if (!found) {
            throw new RefineryError("GRAPH_NODE_NOT_FOUND", `Graph node not found: ${args.nodeId}`, { phase: "graph-inspect", details: { graphPath, nodeId: args.nodeId } });
        }
...
export function planMemoryGraph(args) {
    ...
    try {
        ...
        const addRecord = (record) => {
            if (record)
                records.set(record.node.id, record);
        };
        for (const nodeId of candidateIds)
            addRecord(store.findCurrentNode(nodeId));
        for (const identifier of [...(args.explicitNodeIds ?? []), ...(args.changedNodeIds ?? [])]) {
            addRecord(store.findCurrentNode(identifier));
        }
        if (records.size === 0)
            addRecord(store.findFirstEligibleNode(args.project, args.scope));
        ...
        while (queue.length > 0) {
            ...
            for (const edge of adjacent.edges) {
                if (hydratedEdges.size >= hydratedEdgeLimit)
                    break;
                hydratedEdges.set(edge.id, edge);
                const nextNodeId = edge.sourceNodeId === current.nodeId ? edge.targetNodeId : edge.sourceNodeId;
                if (!records.has(nextNodeId) && records.size < hydratedNodeLimit)
                    addRecord(store.findCurrentNode(nextNodeId));
                if (!records.has(nextNodeId))
                    continue;
                const nextDepth = current.depth + 1;
...
export function readMemoryGraph(args) {
    const store = graphStore(args);
    try {
