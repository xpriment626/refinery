...
const sourceReaderProtocolVersion = "refinery.source-reader.v1";
function globRoot(patternInput) {
...
function sourceReadRoots(options) {
    const roots = new Set();
    for (const spec of options.sourceSpecs) {
        switch (spec.kind) {
            ...
            case "file":
                if (spec.value)
                    roots.add(path.resolve(spec.value));
                break;
            case "glob":
                if (spec.value)
                    roots.add(globRoot(spec.value));
                break;
        }
    }
    return [...roots].sort();
...
function validateCorpus(value, options) {
...
async function readGenericSourceCorpusIsolated(options, readerOptions = {}) {
    ...
    const calculatedMaximum = options.sourceSpecs.length * options.limits.sourceLimit * options.limits.documentCharLimit * 1.25
        ...
        + 1_000_000;
    const maxResponseBytes = Math.max(1_000_000, Math.min(256 * 1024 * 1024, readerOptions.maxResponseBytes ?? calculatedMaximum));
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            child.kill("SIGKILL");
            finish(new RefineryError("SOURCE_READER_TIMEOUT", `Read-only source reader exceeded ${timeoutMs}ms.`, { phase: "source-reader", details: { timeoutMs } }));
        }, timeoutMs);
        child.on("error", (error) => finish(new RefineryError("SOURCE_READER_START_FAILED", `Could not start read-only source reader: ${error.message}`, { phase: "source-reader" })));
        child.stdout.on("data", (chunk) => {
            stdoutBytes += chunk.length;
            if (stdoutBytes > maxResponseBytes) {
                child.kill("SIGKILL");
                finish(new RefineryError("SOURCE_READER_RESPONSE_TOO_LARGE", `Read-only source reader exceeded its ${maxResponseBytes}-byte response limit.`, { phase: "source-reader", details: { maxResponseBytes } }));
                return;
            }
            stdout.push(chunk);
        });
        ...
        child.on("close", (code, signal) => {
            ...
            try {
                ...
                finish(new RefineryError("SOURCE_READER_PROTOCOL_ERROR", `Read-only source reader returned invalid JSON: ${error instanceof Error ? error.message : String(error)}`, { phase: "source-reader", details: { code, signal, stderr: Buffer.concat(stderr).toString("utf8").slice(0, 2_000) } }));
                return;
            }
            ...
            if (!response.ok || !response.corpus || !response.isolation) {
                finish(new RefineryError(response.error?.code ?? "SOURCE_READER_FAILED", response.error?.message ?? `Read-only source reader exited with code ${code ?? "unknown"}.`, { phase: response.error?.phase ?? "source-reader", details: response.error?.details }));
                return;
            }
            try {
                ...
                finish(new RefineryError("SOURCE_READER_PROTOCOL_ERROR", error instanceof Error ? error.message : String(error), { phase: "source-reader" }));
            }
        });
        child.stdin.end(`${JSON.stringify(request)}\n`);
    });
}
function emptyIsolation() {
    return {
...
export async function readSourceCorpusIsolated(options, readerOptions = {}) {
    ...
    if (sessionEntries.length === 0)
        return readGenericSourceCorpusIsolated(options, readerOptions);
    const { loadCodexSessionsFromCatalogue } = await import("../sources/codex-session-catalogue.js");
    ...
    const indexBySourceSetId = new Map();
