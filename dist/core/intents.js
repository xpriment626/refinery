...
export function parseReviewIntent(value) {
    ...
    if (!reviewIntents.includes(candidate)) {
        throw new RefineryError("INVALID_OPTION", `review --intent must be one of: ${reviewIntents.join(", ")}.`, { phase: "args", details: { intent: value } });
    }
    return candidate;
}
export function describeReviewIntent(intent) {
    switch (intent) {
        case "general-review":
