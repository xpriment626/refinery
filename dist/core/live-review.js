...
function parseTargetMemoryIds(value, label) {
    ...
    if (Array.isArray(value)) {
        const ids = value.filter((item) => typeof item === "string" || typeof item === "number");
...
export function parseClaimScout(raw) {
    const parsed = extractJson(raw);
    if (!Array.isArray(parsed.candidates))
        throw new Error("Claim Scout output must contain candidates array.");
    return {
        candidates: parsed.candidates.map((candidate, index) => {
            ...
            if (typeof c.claim !== "string" || !c.claim.trim())
                throw new Error(`Candidate ${index} missing claim.`);
            if (!Array.isArray(c.source_refs))
                throw new Error(`Candidate ${index} missing source_refs array.`);
...
export function parseProposalEditor(raw) {
    const parsed = extractJson(raw);
    if (!Array.isArray(parsed.typed))
        throw new Error("Proposal Editor output must contain typed array.");
    return {
        typed: parsed.typed.map((item, index) => {
            if (!item || typeof item !== "object")
                throw new Error(`Typed item ${index} must be an object.`);
            const t = item;
            if (typeof t.body !== "string" || !t.body.trim())
                throw new Error(`Typed item ${index} missing body.`);
            if (typeof t.memory_type !== "string" || !t.memory_type.trim())
                throw new Error(`Typed item ${index} missing memory_type.`);
            if (typeof t.primary_type !== "string" || !t.primary_type.trim())
                throw new Error(`Typed item ${index} missing primary_type.`);
            ...
            if (!Array.isArray(t.ambiguities))
                throw new Error(`Typed item ${index} missing ambiguities array.`);
            if (t.durability !== "durable" && t.durability !== "ttl" && t.durability !== "ephemeral") {
                throw new Error(`Typed item ${index} has invalid durability.`);
            }
            if (t.ttl !== null && typeof t.ttl !== "string")
                throw new Error(`Typed item ${index} ttl must be string or null.`);
            ...
            if (!Array.isArray(t.source_refs))
                throw new Error(`Typed item ${index} missing source_refs array.`);
            return {
                body: t.body,
...
export function parseDecisionSynthesizer(raw) {
    const parsed = extractJson(raw);
    ...
    const output = {
        proposals: parsed.proposals.map((item, index) => {
            ...
            const p = item;
            if (typeof p.memory_type !== "string" || !p.memory_type.trim())
                throw new Error(`Proposal ${index} missing memory_type.`);
            ...
            if (typeof p.body !== "string" || !p.body.trim())
                throw new Error(`Proposal ${index} missing body.`);
        ...
        rejected: parsed.rejected.map((item, index) => {
            if (!item || typeof item !== "object")
                throw new Error(`Rejected item ${index} must be an object.`);
            const r = item;
            const explanation = item.rejection_rationale
                ?? item.rationale
            ...
            const reason = typeof r.reason === "string" && r.reason.trim()
                ...
                : typeof explanation === "string" && explanation.trim()
                    ...
                    : null;
            if (!reason)
                throw new Error(`Rejected item ${index} missing reason.`);
...
export function parseEvidenceFindings(raw) {
    ...
    if (!Array.isArray(parsed.findings))
        throw new Error("Evidence finding output must contain findings array.");
    return {
        findings: parsed.findings.map((item, index) => {
            ...
            if (typeof f.body !== "string" || !f.body.trim())
                throw new Error(`Finding ${index} missing body.`);
            ...
            if (typeof f.rationale !== "string" || !f.rationale.trim())
                throw new Error(`Finding ${index} missing rationale.`);
            if (!Array.isArray(f.source_refs))
                throw new Error(`Finding ${index} missing source_refs array.`);
            if (!Array.isArray(f.memory_refs))
                throw new Error(`Finding ${index} missing memory_refs array.`);
            ...
            return { ...f, memory_refs: memoryRefs };
        }),
    };
}
...
//# sourceMappingURL=live-review.js.map