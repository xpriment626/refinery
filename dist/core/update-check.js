...
export function compareVersions(left, right) {
    ...
    for (const field of ["major", "minor", "patch"]) {
        if (leftParsed[field] !== rightParsed[field])
            return leftParsed[field] - rightParsed[field];
    }
...
function readCachedResult(options, now) {
    try {
        const cached = JSON.parse(fs.readFileSync(options.cachePath, "utf8"));
        if (cached.currentVersion !== options.currentVersion ||
...
function writeCache(options, checkedAt, latestVersion) {
    try {
...
function versionFromRegistryPayload(payload) {
    if (typeof payload === "string")
        return parseVersion(payload) ? payload.trim() : null;
...
export function formatUpdateNotice(packageName, result) {
    return [
