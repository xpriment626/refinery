...
import { projectKeyForPath } from "../core/paths.js";
import { createGatewayServer } from "./server.js";
import { gatewayStateSchemaVersion } from "./lifecycle.js";
import { createBoundedGatewayLogger } from "./logging.js";
...
function readBootstrap() {
    ...
    if (!stat.isFile() || stat.isSymbolicLink())
        throw new Error("gateway bootstrap must be a regular file");
...
const cleanup = () => {
    ...
    if (fs.existsSync(bootstrap.statePath)) {
        try {
            const state = JSON.parse(fs.readFileSync(bootstrap.statePath, "utf8"));
            if (state.instanceId === bootstrap.instanceId)
                fs.rmSync(bootstrap.statePath, { force: true });
        }
        catch {
