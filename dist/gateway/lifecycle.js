...
export const gatewayStateSchemaVersion = "refinery.gateway-state.v1";
...
export function buildGatewayEnvironment(environment = process.env) {
    const selected = {};
...
async function acquireLifecycleLock(gatewayDir) {
    ...
    throw new RefineryError("GATEWAY_LIFECYCLE_IN_PROGRESS", "Another gateway lifecycle operation is still in progress. Retry after it finishes.", { phase: "gateway-lifecycle", details: { next: "refinery gateway status --json" } });
}
...
function readState(statePath) {
    ...
    try {
        ...
        if (!stat.isFile() || stat.isSymbolicLink())
            throw new Error("gateway state is not a regular file");
        ...
        throw new RefineryError("GATEWAY_STATE_INVALID", `Could not read gateway state at ${statePath}: ${error instanceof Error ? error.message : String(error)}`, { phase: "gateway-lifecycle", details: { statePath } });
    }
}
async function probe(state, timeoutMs = 500) {
    try {
...
function recoverStaleState(statePath) {
    if (!fs.existsSync(statePath))
        return;
...
export async function startGateway(options) {
    ...
    try {
        ...
        try {
            ...
            if (!(error instanceof RefineryError) || error.code !== "GATEWAY_STATE_INVALID")
                throw error;
        ...
        if (existing && await probe(existing)) {
            if (existing.projectKey !== projectKeyForPath(project)) {
                throw new RefineryError("GATEWAY_PROJECT_CONFLICT", `Gateway is already serving ${existing.project}. Stop it before starting another project.`, { phase: "gateway-lifecycle", details: { next: "refinery gateway stop --json" } });
            }
        ...
        while (Date.now() < deadline) {
            ...
            if (childFailure.error || child.exitCode !== null)
                break;
        }
        ...
        if (fs.existsSync(bootstrapPath))
            fs.rmSync(bootstrapPath, { force: true });
        throw new RefineryError("GATEWAY_START_TIMEOUT", childFailure.error ? `Gateway process could not start: ${childFailure.error.message}` : "Gateway did not become healthy within 8 seconds.", { phase: "gateway-lifecycle", details: { logPath: paths.gatewayLogPath } });
    }
    finally {
...
export async function stopGateway(options) {
    ...
    try {
        ...
        try {
            ...
            throw new RefineryError("GATEWAY_STOP_FAILED", `Gateway did not accept graceful shutdown: ${error instanceof Error ? error.message : String(error)}`, { phase: "gateway-lifecycle", details: { next: "refinery gateway status --json" } });
        }
        const deadline = Date.now() + 5_000;
        while (Date.now() < deadline) {
            if (!fs.existsSync(paths.gatewayStatePath) || !await probe(state, 150)) {
                if (fs.existsSync(paths.gatewayStatePath))
                    fs.rmSync(paths.gatewayStatePath, { force: true });
                return { running: false, stale: false, publicState: null, uiUrl: null };
            }
            await sleep(50);
        }
        throw new RefineryError("GATEWAY_STOP_TIMEOUT", "Gateway remained healthy after graceful shutdown was requested.", { phase: "gateway-lifecycle" });
    }
    finally {
        releaseLifecycleLock();
    }
}
export async function notifyGatewayGraphSync(options) {
    const project = path.resolve(options.project);
    const paths = resolveRefineryPaths({ home: options.home, cwd: project });
    const state = readState(paths.gatewayStatePath);
    if (!state || state.projectKey !== projectKeyForPath(project) || !await probe(state, 250))
        return false;
    try {
