...
function uniqueGraphItems(items) {
    ...
    for (const item of items) {
        ...
        if (existing.kind !== item.kind
            ...
            throw new RefineryError("GRAPH_SOURCE_CONFLICT", `Canonical graph source was loaded more than once with conflicting content: ${item.sourceKey}`, { phase: "graph-source", details: { sourceAdapter: item.sourceAdapter, sourceKey: item.sourceKey } });
        }
    }
...
function documentSourceKey(document) {
...
function documentProject(document) {
...
function sourceModifiedAt(metadata) {
...
export function buildCodexGraphSnapshot(args) {
    ...
    const rootProjectScope = args.sourceSets.some((sourceSet) => ((sourceSet.spec.kind === "codex:sessions" || sourceSet.spec.kind === "codex:memories")
    ...
    for (const document of args.documents) {
        ...
        if (relPath)
            documentBySourcePath.set(relPath, item);
        const sessionId = stringMetadata(document.metadata, "sessionId");
        if (sessionId && document.role !== "codex-session-responsibility-unit")
            sessionById.set(sessionId, item);
        const skillName = stringMetadata(document.metadata, "skillName");
        if (skillName)
            skillsByName.set(skillName, [...(skillsByName.get(skillName) ?? []), item]);
        if (itemProject === project) {
            edges.push(edge({
                sourceKey: item.sourceKey,
                targetKey: projectKey,
    ...
    for (const memory of args.activeMemories.filter((candidate) => {
        ...
        for (const [skillName, skills] of skillsByName) {
            ...
            if (!new RegExp(`(?:^|\\s)\\$${escaped}(?=\\s|[.,;:!?)]|$)`, "i").test(memory.body))
                continue;
...
//# sourceMappingURL=codex-graph.js.map