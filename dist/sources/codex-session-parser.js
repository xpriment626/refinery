...
export const sessionIndexerSchemaVersion = "refinery.session-indexer.v1";
...
function isHarnessScaffolding(text) {
...
function phaseFor(text) {
    ...
    if (/\b(debug|diagnos|root cause|why (?:is|does)|failure|error|broken|regression)\b/.test(value))
        return "diagnosis";
    if (/\b(implement|build|add|create|refactor|fix|ship|wire|integrat|migrat)\b/.test(value))
        return "implementation";
    if (/\b(test|verify|benchmark|measure|prove|sanity check|audit)\b/.test(value))
        return "verification";
    if (/\b(research|compare|explore|investigate|understand|analyse|analyze)\b/.test(value))
        return "research";
    if (/\b(decide|choose|recommend|plan|design|think through)\b/.test(value))
        return "decision";
    return "continuation";
}
...
export function cwdMatchesFilter(cwd, filter) {
...
export function cwdSetMatchesFilter(cwds, filter) {
...
function timestampFromPrefix(line) {
...
export async function scanSessionScope(filePath, header) {
    const knownHeader = header ?? await readSessionHeader(filePath);
    const stream = fs.createReadStream(filePath);
    const allCwds = new Set(knownHeader.cwd ? [path.resolve(knownHeader.cwd)] : []);
    ...
    const inspect = (line) => {
        ...
        if (!prefix.includes('"type":"turn_context"') && !prefix.includes('"type": "turn_context"'))
            return;
        ...
        if (typeof entry.payload.cwd === "string")
            allCwds.add(path.resolve(entry.payload.cwd));
    };
...
function finalizeDraft(draft, sessionId, ordinal) {
...
function resultFailureSignal(payload) {
    ...
    try {
        ...
        if (exitCode !== null && exitCode !== 0)
            return `tool result failed with exit code ${exitCode}`;
    }
    catch {
...
export async function parseSessionStream(filePath, header) {
    const knownHeader = header ?? await readSessionHeader(filePath);
    ...
    const finalize = () => {
        if (hasMeaningfulContent(current))
            units.push(finalizeDraft(current, knownHeader.sessionId, units.length));
        current = null;
    };
    for await (const line of lines) {
        ...
        if (line.includes('"role":"developer"') || line.includes('"role": "developer"'))
            continue;
        const entry = parseLine(line);
        if (!entry) {
            parseFailures += 1;
        ...
        if (!current)
            current = newDraft({ line: lineNumber, timestamp, cwd: currentCwd, reasons: pendingReasons });
        current.endLine = lineNumber;
        current.endTimestamp = timestamp ?? current.endTimestamp;
        if (currentCwd)
            current.cwdSet.add(path.resolve(currentCwd));
        if (payload.type === "message" && payload.role === "assistant" && payload.phase !== "commentary") {
            ...
            if (outcome && current.assistantOutcomes.length < 6)
                current.assistantOutcomes.push(outcome);
            continue;
        }
        ...
        if (payload.type === "function_call_output" || payload.type === "custom_tool_call_output") {
            ...
            if (signal && current.failureSignals.length < 4)
                current.failureSignals.push(signal);
        }
    }
    finalize();
...
//# sourceMappingURL=codex-session-parser.js.map